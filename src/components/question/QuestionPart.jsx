    ...
    <Container>
      ...
      <div className="utils">
        <div>
          ...
          {user?.email === question?.userEmail && <> <DeleteBtn questionId={question?.id} /> <AnswerBtn questionId={question?.id} user={user} /> </> }
          {user?.email !== question?.userEmail && <AnswerBtn questionId={question?.id} user={user} />}
        </div>
        <p><span className="hide">Hỏi bởi </span> <span className="cool" onClick={() => history.push(`/user/${question?.username}`)}>@{question?.username}</span></p>
      </div>
    </Container>
  )
}
